...
check_docker_compose_compatibility() {
    log_step "检查Docker Compose版本兼容性..."
    
    # 检查是否有docker-compose命令
    if command -v docker-compose &> /dev/null; then
        local compose_version=$(docker-compose --version 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1)
        log_info "检测到docker-compose版本: $compose_version"
        
        # 检查是否是有问题的版本范围 (1.29.x)
        if [[ "$compose_version" =~ ^1\.29\. ]]; then
            log_warning "检测到docker-compose版本 $compose_version 可能存在兼容性问题"
            log_info "尝试使用 'docker compose' 命令替代..."
            
            # 检查新版docker compose命令是否可用
            if docker compose version &> /dev/null; then
                log_success "将使用 'docker compose' 命令"
                export USE_DOCKER_COMPOSE_V2=true
            else
                log_error "docker-compose版本存在兼容性问题，请升级Docker或docker-compose"
                log_info "解决方案:"
                echo "  1. 升级Docker到最新版本"
                echo "  2. 或者卸载并重新安装docker-compose:"
            ...
            export USE_DOCKER_COMPOSE_V2=false
        fi
    elif docker compose version &> /dev/null; then
        log_info "使用Docker Compose V2 (docker compose)"
        export USE_DOCKER_COMPOSE_V2=true
    else
...
cleanup_old_deployment() {
    ...
    # 停止并删除容器
    if docker_compose_cmd -f "$DOCKER_COMPOSE_FILE" ps -q | grep -q .; then
        log_info "停止现有容器..."
...
check_container_status() {
    ...
    while [ $attempt -lt $max_attempts ]; do
        # 使用docker_compose_cmd ps获取状态
        local status=$(docker_compose_cmd -f "$DOCKER_COMPOSE_FILE" ps --format "table {{.Name}}\t{{.State}}" | grep "$container_name" | awk '{print $2}' || echo "not_found")
        ...
        case $status in
            ...
            "Exit"*)
                ...
                docker_compose_cmd -f "$DOCKER_COMPOSE_FILE" logs --tail=20 $container_name
                return 1
                ;;
            "not_found")
...
init_database() {
    ...
    # 启动数据库服务
    log_info "启动数据库服务..."
    docker_compose_cmd -f "$DOCKER_COMPOSE_FILE" up -d mysql redis influxdb
    ...
    if ! check_container_status "mysql" 120; then
        ...
        docker_compose_cmd -f "$DOCKER_COMPOSE_FILE" logs --tail=100 mysql
    ...
    # 创建数据库和用户
    log_info "创建数据库和用户..."
    docker_compose_cmd -f "$DOCKER_COMPOSE_FILE" exec -T mysql mysql -u root -p"$MYSQL_ROOT_PASSWORD" <<EOF
...
deploy_services() {
    ...
    # 构建镜像
    log_info "构建Docker镜像..."
    if ! docker_compose_cmd -f "$DOCKER_COMPOSE_FILE" build --no-cache; then
...
handle_error() {
    ...
    if [[ $exit_code -ne 0 ]]; then
        ...
        # 显示容器状态
        echo "当前容器状态:"
        docker_compose_cmd -f "$DOCKER_COMPOSE_FILE" ps || true
        
        # 显示最近的日志
        echo "最近的错误日志:"
        docker_compose_cmd -f "$DOCKER_COMPOSE_FILE" logs --tail=20 || true
